/**
 * Integration Test: Initial Setup Flow (Facade Level)
 * 
 * Tests keyManager.initialSetup() - the primary onboarding method for new domains.
 * Uses the KeyManager FACADE (not internal modules directly).
 * 
 * Flow tested:
 * 1. Call keyManager.initialSetup(domain)
 * 2. Verify key generation + directory creation
 * 3. Verify metadata creation
 * 4. Verify active KID is set
 * 5. Verify key is immediately usable (sign JWT, get JWKS)
 * 
 * Gap Addressed: initialSetup() had NO facade-level integration test
 */
...
describe('Integration: Initial Setup Flow (Facade)', () => {
    let testPaths;
    let keyManager;
    ...
    describe('Immediate Usability', () => {
        ...
        it('should make public key immediately retrievable', async () => {
            ...
            // Immediately retrieve public key
            const publicKey = await keyManager.getPublicKey(domain, result.kid);
    ...
    describe('Multi-Domain Setup', () => {
        it('should initialize multiple domains independently', async () => {
            ...
            expect(await activeKidStore.getActiveKid(domain1)).toBe(result1.kid);
            expect(await activeKidStore.getActiveKid(domain2)).toBe(result2.kid);
        });
    ...
    describe('Domain Normalization', () => {
        it('should normalize domain before setup', async () => {
            ...
            const activeKid = await activeKidStore.getActiveKid('LOWERCASE_DOMAIN');
            expect(activeKid).toBe(result.kid);
        });
        ...
        it('should handle case-insensitive domain setup', async () => {
            ...
            const activeKid1 = await activeKidStore.getActiveKid('TESTDOMAIN');
